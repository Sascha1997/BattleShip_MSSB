...
public class ControllerKI implements Initializable{
	...
	public void autoFill(ArrayList<Point>points) {
		...
		for(int i=0;i<points.size();i++) {
					...
					for(Node n: gridPaneEnemy.getChildren()) {
						...
						if(rowIndex==null||columnIndex==null||rowIndex.intValue()>=spielFeldGroeße||columnIndex.intValue()>=spielFeldGroeße) {
	...
	public void spielFeldGenerieren() {
		...
		for(int i = 0;i<this.spielFeldGroeße+1;i++) {
			ColumnConstraints cc = new ColumnConstraints((int)380/(spielFeldGroeße+1));
			RowConstraints rc = new RowConstraints((int)380/(spielFeldGroeße+1));
		...
		for(int i=0;i<spielFeldGroeße+1;i++) {
			for (int j = 0; j<spielFeldGroeße+1;j++) {
				
				Button b = new Button();
				b.setId("cell");
				b.setMinHeight((int)365/(spielFeldGroeße+1));
				b.setMinWidth((int)365/(spielFeldGroeße+1));
				if(j==spielFeldGroeße) {
					b.setText(String.valueOf(i));
					b.setDisable(true);
					int schriftgroeße= 120/spielFeldGroeße;
					b.setStyle("-fx-font-size:"+schriftgroeße+";-fx-color: #000000; -fx-background-color:#2E2E2E;");
					
				}
				if(i==spielFeldGroeße) {
