...
import application.ControllerKI;
import application.Files;
import application.Schiff;
...
public class KI extends Thread{
    ...
    private ArrayList<Schiff> shipsKI = new ArrayList<>();
    ...
    public KI(Socket socket, boolean first, int mode, ControllerKI ki, boolean isOffline){
        this.turn = first;
        this.socket = socket;
        this.mode = mode;
        this.ki = ki;
    ...
    public KI(Socket socket, boolean first, int mode, ControllerKI ki, String setup, boolean isOffline){
        this(socket, first, mode, ki,isOffline);
        this.setup = setup;
    }
    ...
    public void run(){
        ...
        try {
            ...
            if(!this.turn){
                ...
                if(!isOffline)this.ki.setSpielFeldGroeﬂe(this.setup);
            }else{
            ...
            KIHelper.patternFinding(this.enemyField, this.availableShipsEnemy);
    ...
    private void play(){
        ...
        if(!this.hit && !this.gameEnd){
            if(this.mode == 2){
                ...
                this.probPoint = KIHelper.getProbabilityPoint(this.probField);
            }else{
                setProbPoint();
            }
            kiShot();
    ...
    private void setUpGame(String[] data) {
        ...
        KIHelper.setArray(this.enemyField, 0);
        KIHelper.setArray(this.probField, 0);
    ...
    private void clearArray(int[][] arr, int digit){
    ...
    private void shotEnemy(String[] data){
        ...
        if(index == -1){
            line = "answer 0";
            if(!isOffline)this.ki.markiereEigenesFeldVorbei(p);
            this.turn = true;
            ...
            if(this.shipsKI.get(index).checkIfDead()){
                ...
                if(!isOffline)this.ki.erhoeheGegnerVersenkt();
                checkIfGameOver();
            }else{
                line = "answer 1";
            }
            if(!isOffline)this.ki.markiereEigenesFeldTreffer(p);
    ...
    private void answers(String[] data){
        ...
        switch(data[1]){
            ...
            case "2": this.hit = false;
                ...
                this.cords.add(new Point(this.probPoint));
                this.hits++;
                if(!isOffline)this.ki.autoFill(setBorders(this.cords, this.enemyField));
                deleteShip();
                this.direction = 0;
                checkIfGameOver();
        }
    ...
    private void removeNeighbours(){
        ...
        for(int i = 0; i < this.neighbours.size(); i++){
            ...
            if(KIHelper.pointOutOfBounds(tempPoint, this.size) || this.enemyField[tempPoint.x][tempPoint.y] == 1){
    ...
    private ArrayList<Point> getSideLength(int i, int j, Point start, int search){
        ...
        while(!KIHelper.pointOutOfBounds(p, this.size) && this.enemyField[p.x][p.y] == search){
    ...
    private Point getMaxPoint(Point one , Point two){
        if(KIHelper.pointOutOfBounds(one, this.size) || this.enemyField[one.x][one.y] == 1) return two;
        if(KIHelper.pointOutOfBounds(two, this.size) || this.enemyField[two.x][two.y] == 1) return one;
    ...
    private ArrayList<Point> setBorders(ArrayList<Point> arr, int[][] field){
                    ...
                    for(int j = 0; j < this.availableShipsKI[i]; j++){
                        ...
                        while(true) {
                            ...
                            while(cords.size() != 5 - i) {
                                ...
                                if(KIHelper.pointOutOfBounds(p, this.size) || this.gameField[p.x][p.y] != 0) continue w2;

                                cords.add(p);
                            }

                            for(Point p : cords) this.gameField[p.x][p.y] = 3;
                            this.shipsKI.add(new Schiff(cords, cords.size(), "KI"));
